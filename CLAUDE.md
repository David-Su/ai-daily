...
- 产品决策变更（功能取舍、交互调整、设计修改）和任务进度写入 docs/plan.md 的 `## 技术决策` 和 `## 开发进度`
- 不确定的产品问题先问用户，不要自行决定
- 敏感信息（API Keys、Webhook URLs）通过环境变量管理，不硬编码
- config 有更新需要即时更新对应文档 `docs/tech-spec.md` 中的`## 配置详解`
- 新增或修改 domain prompt 时，同步更新 README 和技术规格中的 prompt 路径说明
- 当需要增加兼容性代码的时候，先询问用户是否需要
- 代码生成规范:
    - 代码美观、精简与易读性是优先级最高
    - 不需要考虑特别极端的情况
    - 遵守Python代码规范
- 对 `/prompts` 中的提示词修改规范：
  - 对生成内容的同一种要求只需要声明一次
  - 修改不能像打补丁一样禁止生成以往错误的内容，而应该是修改成明确的要求
