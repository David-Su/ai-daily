...
# 可选兜底端点（llm.fallback.apiKeyName）
# OPENROUTER_API_KEY=your_fallback_key_here
...
如果你使用 OpenRouter、OpenAI 或其他兼容服务，同步修改 `config.yaml` 中的 `llm.baseUrl`、`llm.models` 和 `llm.apiKeyName`。可选的 `llm.fallback` 是独立端点，必须自带 `model`、`baseUrl` 和 `apiKeyName`，不继承主接口。
...
首次运行会自动创建 `news-data/` 目录。启动时会先检查 LLM 接口可用性，检查失败则退出。启用远端 OPML 同步后，系统会在抓取循环开始前强制完成一次 RSS 源同步。
...
| `fallback` | mapping | 可选的独立兜底端点，三档共用，不进 `models` |
| `fallback.model` | string | 兜底模型名 |
| `fallback.baseUrl` | string | 兜底 OpenAI 兼容接口地址，不包含 `/chat/completions` |
