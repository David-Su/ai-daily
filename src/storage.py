...
def build_dedupe_entry(entry: Dict) -> Dict[str, str]:
    """把普通条目转换为去重 entry"""
    return {
        "link": entry.get("link") or "",
        "title": normalize_dedupe_text(entry.get("title", "")),
        "content": normalize_dedupe_text(entry.get("content", "")),
    }
...
def _load_recent_fetch_entries(filepath: str, threshold: int = 150) -> List[Dict]:
    """按 load_existing_links 相同的跨天策略读取近期 fetch 条目
    ...
    if current_minutes < threshold:
        ...
        data_dir = str(Path(filepath).parent) if filepath else "news-data"
        files.append(get_fetch_file(yesterday, data_dir))

    entries = []
    seen_files = set()
    for file in files:
        if not file or file in seen_files or not Path(file).exists():
            continue
        seen_files.add(file)
...
def load_existing_dedupe_keys(filepath: str, threshold: int = 150) -> List[Dict[str, str]]:
    """加载近期条目的去重指纹 entry"""
    entries = _load_recent_fetch_entries(filepath, threshold)
    dedupe_entries = []
    for entry in entries:
        dedupe_entry = build_dedupe_entry(entry)
        if dedupe_entry["link"] or dedupe_entry["title"] or dedupe_entry["content"]:
            dedupe_entries.append(dedupe_entry)
    return dedupe_entries


def is_duplicate_entry(entry: Dict, dedupe_keys: List[Dict[str, str]]) -> bool:
    """判断条目是否命中链接、标题或内容去重 entry"""
    current = build_dedupe_entry(entry)

    return any(
        bool(current["link"] and current["link"] == existing.get("link"))
        or bool(current["title"] and current["title"] == existing.get("title"))
        or bool(current["content"] and current["content"] == existing.get("content"))
        for existing in dedupe_keys
    )


def find_rapidfuzz_duplicate_entry(
        entry: Dict,
        dedupe_keys: List[Dict[str, str]],
        threshold: int,
):
    """用 RapidFuzz 按正文内容相似度判断重复。"""
    content_key = normalize_dedupe_text(entry.get("content", ""))
    if not content_key:
        return None

    for existing in dedupe_keys:
        existing_content = existing.get("content")
        if existing_content and fuzz.ratio(content_key, existing_content) >= threshold:
            return existing
    return None


def add_entry_dedupe_keys(entry: Dict, dedupe_keys: List[Dict[str, str]]):
    """把条目的链接、标题和内容指纹作为单个 entry 加入去重数组"""
    dedupe_entry = build_dedupe_entry(entry)
    if dedupe_entry["link"] or dedupe_entry["title"] or dedupe_entry["content"]:
        dedupe_keys.append(dedupe_entry)
